...
def generate_dummy_data(file_path, num_samples=1000):
    ...
    data = {
        ...
        'no2': np.random.uniform(5, 100, num_samples),
        ...
        'speed': np.random.uniform(10, 80, num_samples)
    }
    df = pd.DataFrame(data)
    # Simple linear combination with some noise to represent AQI
    df['aqi'] = (df['pm25'] * 0.5 + df['pm10'] * 0.3 + df['no2'] * 0.1 + df['co'] * 5 + 
                 (df['vehicle_count'] / 10) - df['speed'] * 0.2 + np.random.normal(0, 10, num_samples))
...
def train_and_save_model():
    ...
    features = ['pm25', 'pm10', 'no2', 'co', 'temperature', 'humidity', 'vehicle_count', 'speed']
    target = 'aqi'
